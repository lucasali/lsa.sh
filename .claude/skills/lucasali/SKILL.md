---
name: lucasali
description: Lucas Ali's opinionated tooling and conventions for JavaScript/TypeScript projects. Use when setting up new projects, configuring ESLint/Prettier alternatives, monorepos, library publishing, or when the user mentions Lucas Ali's preferences.
metadata:
  author: Lucas Ali
  version: "2026.02.03"
---
...
When completing tasks, run `bun run lint --fix` to format the code and fix coding style.
...
{
  "simple-git-hooks": {
    "pre-commit": "bun i --frozen-lockfile --ignore-scripts --offline && npx lint-staged"
  },
...
Use workspaces for monorepo management. Bun uses a simpler approach with `package.json` workspaces field:
...
For dependency version management, consider using consistent versions across workspaces.
...
| Monorepo            | Bun workspaces, centralized alias, Turborepo                    | [monorepo](references/monorepo.md)                       |
